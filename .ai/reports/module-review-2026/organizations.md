...
- ~~M2: Convites duplicados para mesmo email~~ — **CORRIGIDO 2026-08-25**: `findPendingInvitationByEmail(organizationId, email)` adicionado ao port e implementado no Prisma repository (filtra `usedAt: null, revokedAt: null, expiresAt >= now`). Invite use case revoga convite pendente existente antes de criar novo. Spec atualizado com teste de revogação + mock adicionado a todos os specs que usam o repository.
...
- ~~M5: `GET /members` exige `MEMBERS_MANAGE` — exclui roles que deveriam ter acesso~~ — **CORRIGIDO 2026-08-25**: `MEMBERS_VIEW` capability adicionada ao enum e atribuída a todos os roles (OWNER, ADMIN, FINANCE, EVENT_MANAGER, CHECK_IN_STAFF). `GET /members` agora usa `@RequireCapability(MEMBERS_VIEW)` em vez de `MEMBERS_MANAGE`.
- ~~M6: `DELETE /members/:id` retornando 200 em vez de 204~~ — **CORRIGIDO 2026-08-25**: `@HttpCode(204)`, `Promise<void>` e remoção do `return { success: true }` no handler `removeOrganizationMember`.
