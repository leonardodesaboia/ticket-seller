...
- ~~M2: N+1 em `expireActiveReservations`~~ — **CORRIGIDO 2026-08-25**: reescrito com dois SQL em vez de loop: (1) `UPDATE reservations ... RETURNING id` para expirar em bulk; (2) `UPDATE ticket_inventory ... FROM (SELECT SUM(quantity) GROUP BY ticket_type_id, organization_id)` para decrementar reserved em uma passagem. De O(R × (3 + T)) queries para O(2) queries.
- ~~M5–M6: N+1 em revogação de credenciais e outbox por ticket~~ — **CORRIGIDO 2026-08-25**: M5: `UPDATE ticket_credentials ... WHERE ticket_id = ANY(${ids}::uuid[])` — loop substituído por bulk UPDATE. M6: `Prisma.join(rows)` com `Prisma.sql` para bulk INSERT de outbox events em uma única query parametrizada.
- ~~M9: `REFUNDED` ausente do tipo `OrderStatus`~~ — **CORRIGIDO 2026-08-25**: `'REFUNDED'` adicionado ao union type `OrderStatus` em `order.entity.ts`.
