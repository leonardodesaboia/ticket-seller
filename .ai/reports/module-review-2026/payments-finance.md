...
- ~~A7: Race condition em `create-payment-attempt` (constraint `UNIQUE PARTIAL` faltando)~~ — **CORRIGIDO 2026-08-25**: migration `20260825000039` adiciona `CREATE UNIQUE INDEX payment_attempts_unique_active_per_order ON payment_attempts (order_id) WHERE status IN ('PENDING', 'PROCESSING')`. Use case envolve `save()` em try/catch; P2002 no índice parcial → `PaymentAlreadyActiveError`.
