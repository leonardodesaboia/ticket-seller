...
export class PrismaPaymentChargebackOperationAdapter {
  constructor(
  ...
  async process(input: ProcessChargebackInput): Promise<void> {
    ...
    // An existing OPEN dispute represents an interrupted delivery and must be
    // retried; only a PROCESSED dispute is terminal.
    if (inserted === 0) {
      const disputes = await this.prisma.$queryRaw<Array<{ status: string }>>`
        SELECT status FROM payment_disputes
        WHERE provider = ${provider} AND external_dispute_id = ${providerEventId}
        LIMIT 1
      `;
      if (disputes[0]?.status === 'PROCESSED') {
        this.logger.log(`Chargeback ${providerEventId} already processed, skipping`);
        return;
      }
      this.logger.warn(`Retrying unfinished chargeback ${providerEventId}`);
    }
