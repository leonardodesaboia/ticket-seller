...
import type { IPaymentChargebackOperationPort } from '../../application/ports/payment-chargeback-operation.port';
import {
  IFinancialRecordPort,
} from '../../../finance/contracts/financial-record.contract';
import { ILogger } from '../../../../shared/kernel/logger.port';
import type { IPaymentWebhookOperationPort } from '../../application/ports/payment-webhook-operation.port';
...
export class PrismaPaymentWebhookOperationAdapter implements IPaymentWebhookOperationPort {
  constructor(
    private readonly gateway: PaymentGatewayPort,
    private readonly prisma: PrismaService,
    private readonly processChargeback: IPaymentChargebackOperationPort,
  ...
  async process(input: ProcessWebhookInput): Promise<void> {
    ...
    // A duplicate is terminal only after its effects were committed. An earlier
    // delivery may have failed between recording the event and processing it.
    if (insertResult === 0) {
      const events = await this.prisma.$queryRaw<Array<{ processed_at: Date | null; failed_at: Date | null }>>`
        SELECT processed_at, failed_at FROM payment_webhook_events
        WHERE provider = ${input.provider} AND provider_event_id = ${providerEventId}
        LIMIT 1
      `;
      if (events[0]?.processed_at || events[0]?.failed_at) {
        this.logger.log(`Webhook ${providerEventId} already finalized, skipping`);
        return;
      }
      this.logger.warn(`Retrying unfinished webhook ${providerEventId}`);
    }
