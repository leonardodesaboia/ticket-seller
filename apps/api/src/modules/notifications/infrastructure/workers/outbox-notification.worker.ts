...
export class OutboxNotificationWorker implements OnModuleInit, OnModuleDestroy {
  ...
  private async poll(): Promise<void> {
    ...
    this.isPolling = true;
    try {
      // Claim pending events under a short-lived lock. The connection is released
      // immediately after the SELECT so it is not held during email delivery.
      // Each email is sent and marked outside the transaction; the existing
      // notificationLog dedup in SendEmailUseCase prevents double-sends on retry.
      const rows = await this.prisma.$transaction((tx) =>
        tx.$queryRaw<OutboxEventRow[]>`
