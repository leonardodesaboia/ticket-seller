...
- catálogo público: GET /api/v1/public/events (listagem keyset startsAt ASC, id ASC, cursor opaco, PUBLISHED em andamento/futuros) e GET /api/v1/public/events/:slug (detalhe PUBLISHED, acessível após término), sem autenticação, DTOs públicos independentes (evento/venue público/ticket types ativos por preço), onlineInfo/endereço/IDs/capacidade nunca expostos, 404 indistinguível, Cache-Control public max-age=60 swr=300 — TASK-023 (revisada, aguardando integração em develop).
